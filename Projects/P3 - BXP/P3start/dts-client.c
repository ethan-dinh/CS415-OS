...
int main(int argc, char *argv[]) {
    ...
    while (fgets(buf, sizeof(buf), stdin) != NULL) {
        ...
        if (strcmp(w[0], "OneShot") == 0) {
            ...
            if (N > 1) sscanf(w[1], "%ld", &secs);
            tv.tv_sec+= secs;
            sprintf(query, "OneShot|%d|%ld|%ld|%s|DTS-ALARM|%u", id, tv.tv_sec, tv.tv_usec, ipaddr, (unsigned)svcPort);
            ...
            tv.tv_sec += secs;
            sprintf(query, "Repeat|%d|%ld|%ld|%ld|%ld|%s|DTS-ALARM|%u", id, tv.tv_sec, tv.tv_usec, msecs, repeats, ipaddr, (unsigned)svcPort);
