...
/*
 * p1putchr - display character on file descriptor
 */
void p1putchr(int fd, int c) {
    char b = c;
    write(fd, &b, 1);
}
 ...
 */
bool p1strneq(const char *s1, const char *s2, int n){
    int i;

    for (i = 0; i < n; i++) {
        if (s1[i] != s2[i])
            return false;	/* chars at index i are unequal */
    }
    return true;
}
